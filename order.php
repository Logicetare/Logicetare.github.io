	...
	in_array($_GET['method'], $allowedMethods) && Is_Numeric($_GET['id']))
{
	...
	if ($_GET['method'] == "paypal") {
		$paypalurl = "https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&amount=".urlencode($row['price'])."&business=".urlencode($paypalemail)."&item_name=".urlencode($row['name'])."&item_number=".urlencode($row['ID']."_".$_SESSION['ID'])."&return=http://purestress.net/purchase.php"."&rm=2&notify_url=http://purestress.net/gateway/paypalipn.php"."&cancel_return=http://purestress.net/purchase.php"."&no_note=1&currency_code=USD";
 ...
 <form action="" method="POST" id="payment-form">
	...
	<button type="submit" class="submit-button btn blue inverse">Submit Payment</button>
		...
		$button = $cb->createButton($row['name'], $row['price'], "USD", json_encode(array(
			...
			"callback_url" => "http://purestress.net/gateway/bitcoin_ipn.php",
			"key"          => $secret,
