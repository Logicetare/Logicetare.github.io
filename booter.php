...
if (isset($_POST['attackBtn']))
{
	if ($stats->runningBoots($odb) < $maxBootSlots) {
							...
							if ($SQLGetTime->rowCount() != 0) {
								...
								if ($allowedMethod) {
									if (!($time > $maxTime) && $displayCountdown !== true)
									{
										...
										if ($useRotations) {
											$getServers = $odb->query("SELECT * FROM `servers` WHERE `active`='1' ORDER BY ABS(`last_used`) ASC LIMIT 1;");
											if ($getServers->rowCount() != 0) {
												$servers = $getServers->fetchAll(PDO::FETCH_ASSOC);
												$name = $servers[0]['name'];
											}
										} else {
											$getServers = $odb->query("SELECT * FROM `servers` WHERE `active`='1' ORDER BY ABS(`last_used`) ASC;");
										...
										if (!empty($servers)) {
											...
											foreach ($servers as $k => $server)
											{
												...
												$addr = null;
												$updateServer = $odb->prepare("UPDATE `servers` SET `last_used` = UNIX_TIMESTAMP(NOW()) WHERE `id`=:id");
		...
		<div class=\"g_12\"><div class=\"alert alert-success\">SUCCESS: Attack has been sent to " . $host.":".$port . "for " . $time . " seconds using " . $method . " which will be handled by " . ucfirst($name) . ". You can send another attack<span class=\"countdown2\"></span></div></div>";
										} else {
...
<a href="globalstatus.php" class="btn">Global Attacks Status</a>
				<!-- /main-header -->
<!-- /row -->
		<?php
if (isset($_POST['stopBtn']))
				{
					...
					ini_set('default_socket_timeout', 5);
							...
							if (isset($row['stopped']) && $row['stopped'] == "No") {
								...
								echo '<div class="g_12"><div class="alert alert-success"> Attack has been stopped on the server <strong>'.ucfirst($row['server_used']) . '</strong></div></div>';										
									}
						...
						<article class="widget widget__form">
							<header class="widget__header">
								<div class="widget__title" style="width:100%;">
									<i class="pe-7s-menu"></i><h3>Booter</h3>
								</div>
							</header>
...
?>
					...
					<div class="col-md-8">
						<article class="widget">
							<header class="widget__header">
								<div class="widget__title" style="width:100%;">
									<i class="pe-7s-clock"></i><h3>Server Ping</h3>
								</div>
							</header>
										...
										timer = setInterval(function() {
											...
											$.ajax({
												...
												if (data.code === 0 ) $("div.ping-tool").prepend("<div style='color:lime;'>[" + 
													i + " / 25] " +  data.message + "</div>");
												if (data.code !== 0 || i > 24) {
													$("div.ping-tool").prepend("<div style='color:red;'>[" + i + " / 25] " + 
					...
					<div class="col-md-<?php echo (isset($token) ? "12" : "8"); ?>">
						<article class="widget">
							...
							<div class="tab-radio-full">
								...
								<label for="tab_radio_1" class="btn-md">Current Boots</label>
								<input type="radio" id="tab_radio_2" name="tab-radio-2" value="tabr2" >
								<label for="tab_radio_2" class="btn-md">Previous boots</label>
							</div>
