...
$SQLGetTickets = $odb -> prepare("SELECT * FROM `tickets` WHERE `id` = :id AND `username` = :user");
...
echo '<div class="g_12"><div class="alert alert-success">SUCCESS: Ticket has been closed.  Redirecting....</div></div><meta http-equiv="refresh" content="3;url=support.php">';
	   ...
	   {
		if ($closed) {
			echo '<div class="alert alert-error"><p><strong>ERROR:</strong> Ticket has been closed, please create a new ticket if you need further assistance.</div>';
		} else {
			...
			if (empty($errors))
			{
				$SQLinsert = $odb -> prepare("INSERT INTO `messages` VALUES(NULL, :ticketid, :content, :sender, UNIX_TIMESTAMP(NOW()))");
			$SQLinsert -> execute(array(':sender' => $_SESSION['username'], ':content' => $updatecontent, ':ticketid' => $id));
			{
				$SQLUpdate = $odb -> prepare("UPDATE `tickets` SET `status` = :status WHERE `id` = :id");
				$SQLUpdate -> execute(array(':status' => 'Waiting for Staff response.', ':id' => $id));
...
?>
											<div class="media message ticket-msg-<?php echo $row['messageid'] . ($isStaff ? " fav msg-staff" : " msg-user"); ?>">
												...
												<div class="media-body">
													<h4 class="media-heading message__heading"><?php echo htmlentities($row['sender']); ?> <span><?php echo ($isStaff ? '<span class="badge badge--red">PureStress Staff</span>' : "") . (isset($row['date']) && !is_null($row['date']) ? timeElapsedFromUNIX($row['date']) : "n/a"); ?></span></h4>
...
<?php
	...
	?>
											<div class="alert alert-fixed alert-danger alert-dismissible" role="alert">
											  ...
											  <p class="alert__text"> Notice: This ticket has been closed by a <?php include 'includes/name.php'; ?> Staff, if you need further assistance please open a new ticket.</p>
											</div>
<?php	
											}
?>
